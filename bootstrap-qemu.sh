...
else sudo qemu-system-x86_64 -boot c -cdrom "win2022.iso" -hda disk.qcow2 -m $MEMORY -M pc -machine accel=kvm -enable-kvm -cpu host -smp $CPUS,cores=1 -vnc :99 -usb -net nic -no-user-config -nodefaults -rtc base=localtime -boot strict=on -chardev pty,id=charserial0 -device isa-serial,chardev=charserial0,id=serial0 -k en-us -device qxl-vga,id=video0,ram_size=67108864,vram_size=67108864,vram64_size_mb=0,vgamem_mb=1531,max_outputs=1 -msg timestamp=on -soundhw hda -spice port=5900,addr=127.0.0.1,disable-ticketing -usb -usbdevice tablet -L /usr/share/qemu &
fi
sleep 5;
